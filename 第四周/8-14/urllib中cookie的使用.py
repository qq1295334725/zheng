...
# # 第二步是根据cookiejar对象创建cookie信息的管理对象handler
...
#
# MozillaCookieJar和LWPCookieJar都是用于将cookie信息保存到本地文件的一种形式，区别在于使用MozillaCookieJar生成的Cookie信息会保存为Mozilla类型的Cookie格式。使用LWPCookieJar会将cookie信息保存为libwww-perl格式的cookie文件。


# 第一步：声明一个MozillaCookieJar或者LWPCookieJar的一个对象
cookie_mozilla_obj = http.cookiejar.MozillaCookieJar(filename="cookie.txt")
# 第二步：根据MozillaCookieJar生成的对象cookie_mozilla_obj，来创建一个cookie信息的管理对象handler
handler = urllib.request.HTTPCookieProcessor(cookie_mozilla_obj)
# 第三步：根据handler对象创建opener对象
opener = urllib.request.build_opener(handler)
# 第四步：根据opener对象对网址发起请求
response_test = opener.open("http://baidu.com")
# 第五步：将cookie信息保存到本地
# ignore_discard=True，即使cookie信息将要过期/作废，也要将其保存到文件
# ignore_expires=True，即使cookie信息在文件中已经存在，仍然对其进行覆盖写入
cookie_mozilla_obj.save(ignore_expires=True,ignore_discard=True)


# -----------读取cookie信息对网站进行访问------
# 第一步：生成一个MozillaCookieJar对象
cookie_obj = http.cookiejar.MozillaCookieJar()
# 第二步：加载本地的cookie信息
# ignore_discard=True，即使cookie信息将要过期/作废，也仍要读取cookie信息
# ignore_expires=True，即使cookie信息在文件中已经存在，仍然也要读取
